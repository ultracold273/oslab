...
sys_exofork(void)
{
	...
	// Mark the child env as not runnable
	e->env_status = ENV_NOT_RUNNABLE;
	// Copy the current register state
	e->env_tf = curenv->env_tf;
	// Mark the return value is 0
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	user_mem_assert(curenv, (void *)tf, sizeof(struct Trapframe), PTE_P | PTE_U | PTE_W);
	e->env_tf = *tf;
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// perm checking again
	if ((perm & PTE_W) && !(PGOFF(*srcva_pte) & PTE_W)) {
		return -E_INVAL;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	// Check whether dst env is receiving
	if (!e->env_ipc_recving) {
		return -E_IPC_NOT_RECV;
	}
	// Copy the page mapping
	...
	e->env_tf.tf_regs.reg_eax = 0; // return 0 as successfull
...
sys_ipc_recv(void *dstva)
{
	...
	sched_yield();
	// In fact we cannot get here as when this env gets scheduled
	// it directly returned to user state.
	return 0;
}
